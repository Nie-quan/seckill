...
var seckill = {
    ...
    URL:{
        now : function(){
            return projName + '/seckill/time/now';
        },
        exposer : function (seckillId) {
            return projName + '/seckill/'+seckillId+'/exposer';
        },
        execution : function (seckillId,md5) {
            return projName + '/seckill/'+seckillId+'/'+md5+'/execution';
        }
    },
    // 处理秒杀逻辑
    handleSeckillKill : function (seckillId,node) {
        // 获取秒杀地址，控制显示逻辑，执行秒杀
        node.hide().html('<button class="btn btn-primary btn-lg" id="killBtn">开始秒杀</button>');//隐藏秒杀按钮
            ...
            if(!seckill.validatePhone(killPhone)){
                ...
                $('#killPhoneBtn').click(function(){
                    ...
                    if(seckill.validatePhone(inputPhone)){
                        // 电话写入cookie
                        $.cookie('killPhone',inputPhone,{expires : 7,path : projName + '/seckill'});
                       // 刷新页面
